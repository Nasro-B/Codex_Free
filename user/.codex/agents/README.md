...
1. Copy every `*.toml` in this folder into your Codex agents directory:
   - Windows: `C:\Users\<you>\.codex-free\agents\`
   - macOS/Linux: `~/.codex/agents/`
2. Make sure multi-agent is enabled in `~/.codex-free/config.toml`:
   ```toml
   [features]
   multi_agent = true
   ```
3. A model backend must be reachable (for example the Codex Free EN LiteLLM bridge on `:4200`, or OpenAI). If the backend
